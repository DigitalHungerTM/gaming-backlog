...
def update_game(id):
    ...
    update_game_form.launcher_id.choices = [(l.id, l.name) for l in db.session.scalars(select(Launcher).options(load_only(Launcher.id, Launcher.name)).order_by(Launcher.id))]
    update_game_form.status_id.choices = [(s.id, s.name) for s in db.session.scalars(select(Status).options(load_only(Status.id, Status.name)).order_by(Status.id))]
    update_game_form.proton_id.choices = [(p.id, p.rating) for p in db.session.scalars(select(Proton).options(load_only(Proton.id, Proton.rating)).order_by(Proton.id))]
